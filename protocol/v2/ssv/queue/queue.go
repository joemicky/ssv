...
type priorityQueue struct {
	...
	// queueId is used to provide additional details when recording inboxSizeMaxMetric.
	queueId            string
	inboxSizeMaxMetric metric.Int64Gauge
}

type Option func(*priorityQueue)

func WithInboxSizeMaxMetric(inboxSizeMaxMetric metric.Int64Gauge, queueId string) Option {
	return func(q *priorityQueue) {
		q.queueId = queueId
		q.inboxSizeMaxMetric = inboxSizeMaxMetric
	}
}
...
func (q *priorityQueue) recordInboxSize(inboxSize int64) {
	if q.inboxSizeMaxMetric != nil {
		q.inboxSizeMaxMetric.Record(
			context.Background(),
			inboxSize,
